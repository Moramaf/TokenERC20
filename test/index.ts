...
describe("MyToken", function () {
  ...
  beforeEach(async function () {
    ...
    newToken = await Token.deploy("MyToken", "MTK", ethers.utils.parseUnits("1000", decimals), decimals);
  ...
  it("Should return total supply of tokens from getter totalSupply", async function () {
    expect(await newToken.totalSupply()).to.equal(ethers.utils.parseUnits("1000", decimals));
  });

  it("Checking transfer and balanceOf tokens", async function () {
    await newToken.transfer(addr2.address, ethers.utils.parseUnits("100", decimals));
  ...
  it("checking transferFrom", async function () {
    await newToken.approve(addr2.address, 100000000);
    await newToken.connect(addr2).transferFrom(owner.address, addr3.address, 100000000);
  ...
  it("Revert if not enough tokens", async function () {
    await expect(newToken.transfer(addr2.address, 1100000000)).to.be.revertedWith("not enough token!");
  });

  it("Revert if not enough tokens", async function () {
    await newToken.transfer(addr2.address, 100000000);
    await expect(newToken.transferFrom(addr2.address, addr3.address, 100000000)).to.be.revertedWith("amount is not allowed!");
  });

  it("Revert if not enough tokens for transferFrom function", async function () {
    await newToken.approve(addr2.address, 100000000);
    await expect(newToken.connect(addr2).transferFrom(owner.address, addr3.address, 1100000000)).to.be.revertedWith("not enough token!");
  });
